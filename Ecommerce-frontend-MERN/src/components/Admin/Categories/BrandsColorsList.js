...
import { deleteBrandAction, fetchBrandAction } from '../../../redux/slices/categories/brandSlices';
...
export default function BrandsColorsList() {
  ...
  const{loading, error, brands} = useSelector(state=>state.brand);
  ...
  return (
    ...
    <div className="px-4 sm:px-6 lg:px-8">
      ...
      <h3 className="text-lg font-medium leading-6 text-gray-900 mt-3">
        All Brands [{brands?.brand?.length}]
      </h3>
      {loading ? (
        <LoadingComponent />
      ) : error ? (
